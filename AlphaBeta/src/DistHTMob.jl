module DistHTMob
    ...
    using Heuristics.DistributedHistory, Heuristics.Transposition, Heuristics.Territory, Heuristics.Mobility
    using Board
    using Timers
    ...
    historyRefenceStorage = Dict{Player, DistributedMoveHistory}()

    mutable struct TrackedState
        b::BoardState
        p::Player
        transTable::TranspositionTable
        history::DistributedMoveHistory
        colour::Int
        maxDepth::Int
        timeOffset::Float64
    end
    ...
    function iterativeDeepening(overseer::GameOverseer,b::BoardState, player::Player)
        moveHist = haskey(historyRefenceStorage, player) ? historyRefenceStorage[player] : initializeDistributedMoveHistory()
