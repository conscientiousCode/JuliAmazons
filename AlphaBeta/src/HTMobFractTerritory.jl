module HTMobFractTerritory
    using GameManager, GameManager.Players
    using Heuristics.History, Heuristics.Transposition, Heuristics.FractTerritory, Heuristics.Mobility
    using Board
    using Timers
    ...
    function max(state::TrackedState, currentDepth::Int, alpha::Float64, beta::Float64)::Float64
        ...
        if state.maxDepth == currentDepth
            boardValue = getTerritoryScore(state.b, state.colour)
